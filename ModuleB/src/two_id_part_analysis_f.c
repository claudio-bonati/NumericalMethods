...
void computejack(double * restrict datajack, 
  ...
  double x1signtot, x2signtot, r2signtot, signtot;
  double x1sign, x2sign, r2sign, sign;
  ...
  for(i=0; i<sampleeff; i++)
     {
     x1signtot+=data[5*i+0]*(1.0-2.0*data[5*i+4]); 
     x2signtot+=data[5*i+2]*(1.0-2.0*data[5*i+4]); 
     r2signtot+=(data[5*i+1]+data[5*i+3])*(1.0-2.0*data[5*i+4]); 
  ...
  for(i=0; i<numberofbins; i++)
     ...
     for(j=0; j<binsize; j++)
        ...
        x1sign-=data[5*r+0]*(1.0-2.0*data[5*r+4]);
        x2sign-=data[5*r+2]*(1.0-2.0*data[5*r+4]);
        r2sign-=(data[5*r+1]+data[5*r+3])*(1.0-2.0*data[5*r+4]);
        sign-=(1.0-2.0*data[5*r+4]);
        }

     x1sign/=(double)((numberofbins-1)*binsize); 
     x2sign/=(double)((numberofbins-1)*binsize);
     r2sign/=(double)((numberofbins-1)*binsize);
     sign/=(double)((numberofbins-1)*binsize);
 
     datajack[4*i+0]=x1sign/sign;
     datajack[4*i+1]=x2sign/sign;
     datajack[4*i+2]=r2sign/sign;
     datajack[4*i+3]=sign;
     }
  }
...
int main(int argc, char **argv)
    ...
    if(argc != 4)
      ...
      fprintf(stdout, "Output [Fermionic case]:\n");
