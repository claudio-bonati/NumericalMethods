...
int main(void)
    ...
    double x[10];
    int xi[10];
    char datafile[20]; // this will be the file name
    FILE *fp; // pointer to file, needed to read/write
    ...
    // everything worked properly, by adding the code below
    //if(fp==NULL)
    //  {
    //  fprintf(stderr, "Error in opening the file %s (%s, %d)\n", datafile, __FILE__, __LINE__);
    //  exit(EXIT_FAILURE);  // this abort the execution returning an error code
    //  }
    ...
    for(i=0; i<10; i++)
       ...
       err=fscanf(fp, "%lf %d", &(x[i]), &(xi[i])); // neglecting "err" would result in a warning
                                                    // if everything is ok "err" is equal to 2
